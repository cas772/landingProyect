<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="estilos.css">
    <title>VerdUP</title>
</head>
<body>
<div class="contenedor">
<header>
    <div class="logo">
        <img src="imagenes/log1.svg" alt="logo-de-empresa">
    </div>
    <h1>VerdUP</h1>
    <nav class="botoprincipal">
        <ul>
            <li><a href="acercade.php" class="btns">Acerca de Nosotros</a></li>
            <li><a href="comocomprar.php" class="btns">¿Cómo comprar?</a></li>
            <li><a href="index.php" class="btns">Productos</a></li>
            <li><a href="" class="btns">Contacto</a></li>
        </ul>
    </nav>
</header>
<section>
<div id="descripcionProd">

</div>
<div id="imgProd">
    
</div>
</section>
<aside></aside>
<footer>
    <div class="logo">
        <img src="imagenes/log1.svg" alt="logo-de-empresa">
    </div>
    <div class="redeso">
        <a href="http://"><img src="imagenes/wasaimg.png" alt="Icono enlace a whatsApp"></a>
        <a href="http://"><img src="imagenes/emailimg.png" alt="Icono enlace a correo"></a>
        <a href="http://"><img src="imagenes/emafaceimg.png" alt="Icono enlace a Facebook"></a>
        <a href="http://"><img src="imagenes/instag-img.png" alt="Icono enlace a Instagram"></a>
    </div>
</footer>
</div>
<script type="text/javascript" src="js/code.js"></script>    
</body>
</html>